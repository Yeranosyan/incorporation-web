...
const mockMedia = (matching: string[]) =>
  vi.spyOn(window, "matchMedia").mockImplementation((query) => mediaQuery(matching.includes(query), query));
...
describe("ProductCycle", () => {
  ...
  it("shows the collapsed cycle and every row switched when motion is reduced", () => {
    ...
    const reels = [...container.querySelectorAll<HTMLElement>(".cycle-readout .digit-reel")];
    const shown = reels.map((reel) => reel.children[Number(reel.style.getPropertyValue("--digit"))].textContent);
    expect(shown).toEqual(["1", "2"]);
    const switches = [...container.querySelectorAll<HTMLElement>("[data-row]")].map((row) =>
      row.style.getPropertyValue("--switch"),
    );
