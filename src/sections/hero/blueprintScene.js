...
const lotBox = (i, j, h) => ({ x: i * PITCH, y: j * PITCH, w: LOT, d: LOT, h });

const lotsInFrontOf = ({ i, j }) => [lotKey(i + 1, j), lotKey(i, j + 1), lotKey(i + 1, j + 1)];

const cityBoxes = (i, j, lowered) => {
...
export const buildScene = (modules) => {
  ...
  const city = LOT_GRID.filter(([i, j]) => !reserved.has(lotKey(i, j))).flatMap(([i, j]) =>
    cityBoxes(i, j, lowered.has(lotKey(i, j))).map((box, part) => ({ id: `${lotKey(i, j)}-${part}`, box })),
  );
  ...
  const ground = { x: -UNIT, y: -UNIT, w: SIZE + UNIT * 2, d: SIZE + UNIT * 2 };
