...
export type CycleCollapseOptions = { chart: CycleChart; plan: Seam[] };

type SlabFace = keyof ReturnType<CycleChart["slab"]>;
...
const resting = (value: number): SpringState => ({ value, velocity: 0 });

const settle = (state: SpringState, target: number, config: SpringConfig, seconds: number) => {
...
export const useCycleCollapse = <T extends HTMLElement = HTMLDivElement>({
  chart,
  plan,
}: CycleCollapseOptions) => {
  ...
  useEffect(() => {
    ...
    const parts = (name: string) => [...track.querySelectorAll(`[data-part="${name}"]`)];
    const faces: Record<SlabFace, Element[]> = {
      front: parts("front"),
      top: parts("top"),
      right: parts("right"),
      rim: parts("rim"),
    };
    const seamsPath = track.querySelector<SVGPathElement>("[data-part='seams']");
    const liveSeam = track.querySelector<SVGPathElement>("[data-part='live-seam']");
    const live = track.querySelector<SVGGElement>("[data-part='live']");
    const tag = track.querySelector<HTMLElement>("[data-part='tag']");
    const ghostTag = track.querySelector<HTMLElement>("[data-part='ghost-tag']");
    if (!seamsPath || !liveSeam || !live || !tag || !ghostTag) return undefined;
    const rows = [...track.querySelectorAll<HTMLElement>("[data-row]")];
    ...
    const draw = () => {
      ...
      (Object.entries(faces) as [SlabFace, Element[]][]).forEach(([name, nodes]) =>
        nodes.forEach((path) => path.setAttribute("d", slab[name])),
      );
  ...
  return [trackRef, state, focus] as const;
};
