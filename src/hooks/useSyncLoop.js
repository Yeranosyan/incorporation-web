...
const INITIAL_STATUS = { order: INITIAL_ORDER, read: 0, active: 0, written: 0 };

const heights = new WeakMap();

const place = (node, { cx, cy, top, depth, clip, opacity }, size) => {
  if (!node) return;
...
export const useSyncLoop = (motion) => {
  ...
  useEffect(() => {
    ...
    const slots = Array.from({ length: SYNC_POOL }, (_, slot) => root.querySelector(`[data-slot="${slot}"]`));
    const write = root.querySelector("[data-write]");
