...
const report = (index: number, entry: Partial<IntersectionObserverEntry>) => {
  ...
  act(() => observer.callback([{ target, rootBounds: domRect(BAND), ...entry } as IntersectionObserverEntry]));
};
...
describe("useScrollStep", () => {
  ...
  it("exposes scroll progress through the steps as a CSS variable", async () => {
    ...
    vi.spyOn(marker, "getBoundingClientRect").mockReturnValue(domRect({ top: band - 600, height: 400 }));
    fireEvent.scroll(window);
    await act(() => new Promise((resolve) => requestAnimationFrame(resolve)));
    expect(track.style.getPropertyValue("--scroll-progress")).toBe("1.500");

    vi.spyOn(marker, "getBoundingClientRect").mockReturnValue(domRect({ top: band + 200, height: 400 }));
    fireEvent.scroll(window);
    await act(() => new Promise((resolve) => requestAnimationFrame(resolve)));
    expect(track.style.getPropertyValue("--scroll-progress")).toBe("0.000");
  });
  ...
  it("scrolls a chosen marker into the center band", () => {
    ...
    vi.spyOn(marker, "getBoundingClientRect").mockReturnValue(domRect({ top: 1000, bottom: 1400 }));
  ...
  it("follows the marker under the viewport center band", () => {
    ...
    report(2, { isIntersecting: true, boundingClientRect: domRect({ top: 300, bottom: 700 }) });
    expect(screen.getByRole("status")).toHaveTextContent("2");

    report(1, { isIntersecting: true, boundingClientRect: domRect({ top: 200, bottom: 600 }) });
  ...
  it("holds the first step before the track and the last step after it", () => {
    ...
    report(2, { isIntersecting: true, boundingClientRect: domRect({ top: 300, bottom: 700 }) });
    report(0, { isIntersecting: false, boundingClientRect: domRect({ top: 900, bottom: 1300 }) });
    expect(screen.getByRole("status")).toHaveTextContent("0");

    report(2, { isIntersecting: false, boundingClientRect: domRect({ top: -600, bottom: -200 }) });
    expect(screen.getByRole("status")).toHaveTextContent("2");
  });
});
