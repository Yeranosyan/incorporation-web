...
import type { SyncMotion } from "./syncScene";
import type { Vec2 } from "./isometric";
...
const arrivalStart = (arrival: number) =>
...
export const syncFrameAt = (time: number, motion: SyncMotion) => {
  ...
  const settledAfter = (arrival: number) => {
    let sum = 0;
...
export type SyncFrame = ReturnType<typeof syncFrameAt>;
