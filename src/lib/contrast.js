const wordsOf = (phrase) => phrase.split(" ");

const sharedLength = (first, second, pick) => {
...
const fromStart = (words, index) => words[index];

const fromEnd = (words, index) => words[words.length - 1 - index];

const spanOf = (words) => words.join(" ").length;

const placed = (words, span) => {
  let offset = 0;
...
export const splitContrast = (before, after) => {
