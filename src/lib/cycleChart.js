...
const project = ([x, y, z]) => [round(x + z * DX), round(-(y + z * DY))];

const line = (points) => `M${points.map((point) => project(point).join(" ")).join("L")}`;

const shape = (points) => `${line(points)}Z`;

const faces = ({ x0, x1, y0, y1, z0, z1 }) => ({
  front: shape([
    [x0, y0, z0],
    [x1, y0, z0],
    [x1, y1, z0],
...
export const buildCycleChart = (layout, scale, bands) => {
  ...
  const xPercent = (x, z = 0) => ((project([x, 0, z])[0] - minX) / width) * 100;
  const yPercent = (y, z = 0) => ((project([0, y, z])[1] - minY) / tall) * 100;
  ...
  return {
    ...
    axisPercent: (week) => xPercent(week),
    slab: (left, right, rise) => {
    ...
    seams: (positions, rise) =>
      positions
        ...
        .join(""),
    tagAt: (x, rise) => [xPercent(x, z0), yPercent(rise + height, z0)],
    lift,
  };
};
