...
import type { Point, Vec2 } from "@/lib/isometric";

export type SyncLayout = "wide" | "compact";
...
const LIFT = u(1.5);
const PADDING = u(0.5);
const LAYOUTS: Record<SyncLayout, { span: number; below: number }> = {
  wide: { span: u(10.5), below: u(1.9) },
  compact: { span: u(7), below: u(0.2) },
};

const prism = ({ x, y, z = 0, w, d, h }: Prism): PrismFaces => ({
  top: [
    [x, y, z + h],
    [x + w, y, z + h],
    [x + w, y + d, z + h],
...
const openPath = (points: Point[]) => `M${points.map((point) => project(point).join(" ")).join("L")}`;

const solid = (box: Prism) => {
  const faces = prism(box);
  return { top: toPath(faces.top), left: toPath(faces.left), right: toPath(faces.right) };
};

const square = (cx: number, cy: number, z: number, size: number, h = 0): Prism => ({
  x: cx - size / 2,
  y: cy - size / 2,
  z,
  w: size,
...
export const hangingSides = (size: number, h: number) => {
  const faces = prism(square(0, 0, -h, size, h));
  return { left: toPath(faces.left), right: toPath(faces.right) };
};

const hanging = (size: number, h: number) => ({
  top: toPath(prism(square(0, 0, 0, size)).top),
  ...hangingSides(size, h),
});
...
const planeText = (point: Point, plane: keyof typeof TEXT_PLANES) => {
  const [x, y] = project(point);
  return TEXT_PLANES[plane](x, y);
};

export const buildSyncScene = (layout: SyncLayout) => {
  const { span, below } = LAYOUTS[layout];
  const source = { x: 0, y: 0 };
  const target = { x: span, y: -span };
  const plinthOf = ({ x, y }: Lot): Prism => ({ x, y, z: 0, w: LOT, d: LOT, h: PLINTH });
  const bodyOf = ({ x, y }: Lot): Prism => ({
    x: x + INSET,
    y: y + INSET,
    z: PLINTH,
    w: BODY,
  ...
  const centerOf = ({ x, y }: Lot): Vec2 => [x + LOT / 2, y + LOT / 2];
  ...
  const percentOf = (point: Point): Vec2 => {
    const [x, y] = project(point);
...
export type SyncScene = ReturnType<typeof buildSyncScene>;
